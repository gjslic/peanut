...
class Auction extends ModuleBaseController
{
    ...
    public function vehicle(){
        ...
        if($data){
            //解析数据
            //品牌ID 
            $brandID = getPost()['brandID'] ? getPost()['brandID'] : '';
            //系类ID
            $seriesID = getPost()['seriesID'] ? getPost()['seriesID'] : '';
            // 价格范围
            $price = getPost()['price'] ? getPost()['price'] : '';
            // 排序
            $timeBaseNum = getPost()['timeBaseNum'] ? getPost()['timeBaseNum'] : '';
            // 价格排序
            $priceBaseNum = getPost()['priceBaseNum'] ? getPost()['priceBaseNum'] : '';
            
            // 排序条件
            $order = "";
            // 查询车辆条件
            $where = 'v.series_id=s.series_id and s.brand_id=b.brand_id and v.vehicle_state="拍卖中" and a.id=v.auction_id';
            if($brandID != ''){
            ...
            $vehicle = db('vehicle v,peanut_brand b,peanut_series s,peanut_auction a')->field("v.*,a.*")->where($where)->orderRaw($order)->select();
        }else{
